...
import com.retro.dev.dtos.GalleryDTO;
import com.retro.dev.dtos.MediaLinksDTO;
...
private String createResourceSubFolder(String folderName) throws URISyntaxException, IOException {
	...
	private List<Long> generateUserSubCategoryList(List<UserCategory> userCategoryList) {
		...
		for(UserCategory eachUserCat: userCategoryList) {
			subCatList = subCategoryDao.findByCategory(eachUserCat.getCategory());
    	}
    	if (null!=subCatList) {
    		for(SubCategory eachSubCat : subCatList) {
	...
	private HashMap<Long, String> getCategoryAndSubCategoryMappings(List<UserCategory> userCategoryList) {
		...
		for(UserCategory eachUserCat: userCategoryList) {
			...
			if (null!=subCatList) {
	    		for(SubCategory eachSubCat : subCatList) {
	    			categoryMap.put(eachSubCat.getSubcategoryId(), eachUserCat.getCategory().getCategory());
	    			userSubCatList.add(eachSubCat.getSubcategoryId());
	    		}
	    	}
    	}    	
	...
	private List<Long> getFavouriteServicesForUser(Long uid) {
		List<Long> favList = new ArrayList<Long>();
	    List<Favourites> favourites =  favouritesDao.findAllByUserId(uid);
		for(Favourites fav: favourites) {
			if(fav.getOrderId() !=null) {
				favList.add(fav.getOrderId());
			}
			if(fav.getServiceId() != null) {
				favList.add(fav.getServiceId());
			}
    	}    	
		return favList;
	}
	...
	public GalleryDTO getMediaLinksForGallery(Principal principal, BusinessServicesRequest listBservices) {
    	GalleryDTO dto = new GalleryDTO();
    	List<MediaLinksDTO> data = new ArrayList<>();
        try {
        	List<BusinessServices> bServicesList = new ArrayList<>();
        	if(listBservices.getCategoryId() != null) {        		
        		bServicesList = businessServiceDao.findAllBycategoryIdAndUserId(listBservices.getUserId(), Arrays.asList(listBservices.getCategoryId()));
        	} else {
        		bServicesList = businessServiceDao.findAllByUserId(listBservices.getUserId());
        	}
    		Long maxLinks = listBservices.getMaxLinks();
    		if(maxLinks == null)
    			maxLinks = (long) bServicesList.size();
    			
    		List<UserCategory> userCategoryList = userCategoryDao.findAllByUserId(listBservices.getUserId());
			HashMap<Long, String> categoryMap = getCategoryAndSubCategoryMappings(userCategoryList);
			List<Long> favourites = getFavouriteServicesForUser(listBservices.getUserId());
			
			List<BusinessServicesDTO> bServicesDTOList = new ArrayList<BusinessServicesDTO>();
        	List<BusinessServicesDTO> addedRecentMediaLinks = new ArrayList<>();
        	List<BusinessServicesDTO> topRatedMediaLinks = new ArrayList<>();
        	List<BusinessServicesDTO> favouriteMediaLinks = new ArrayList<>();        	
        	HashMap<String, List<BusinessServicesDTO>> categoryMediaLinks = new HashMap<String, List<BusinessServicesDTO>>();
        	...
        	for(BusinessServices eachService : bServicesList) {
				...
				if(distanceInKM!=-999999 && distanceInKM<=10) {
					...
					BeanUtils.copyProperties(eachService, eachServiceDTO);
					eachServiceDTO = addServiceIconImage(eachServiceDTO);
					if(maxLinks!= null && bServicesDTOList.size() < maxLinks) {
						bServicesDTOList.add(eachServiceDTO);
					} 
					if(maxLinks!= null && addedRecentMediaLinks.size() < maxLinks) {
						addedRecentMediaLinks.add(eachServiceDTO);
					}
					if(maxLinks!= null && topRatedMediaLinks.size() < maxLinks) {
						topRatedMediaLinks.add(eachServiceDTO);
					} 
					if(favourites.contains(eachService.getServiceId())) {
						if(maxLinks!= null && favouriteMediaLinks.size() < maxLinks) {
							favouriteMediaLinks.add(eachServiceDTO);
						}
					}
					List<BusinessServicesDTO> existingList = categoryMediaLinks.get(categoryMap.get(eachService.getCategoryId()));
					if(existingList !=null) {
						if(maxLinks!= null && existingList.size() < maxLinks) {
							existingList.add(eachServiceDTO);
						}
					} else {
						List<BusinessServicesDTO> list = new ArrayList<>();
						if(maxLinks!= null && list.size() < maxLinks) {
							list.add(eachServiceDTO);
						}
						categoryMediaLinks.put(categoryMap.get(eachService.getCategoryId()), list);
					}
				} else {
					BusinessServicesDTO eachServiceDTO = new BusinessServicesDTO();
					BeanUtils.copyProperties(eachService, eachServiceDTO);
					...
					if(existingList !=null) {
						...
						if(categoryMap.get(eachService.getCategoryId()) != null)
								categoryMediaLinks.put(categoryMap.get(eachService.getCategoryId()), list);
					}
				}
			}
			//sort addedRecentMediaLinks && topRatedMediaLinks in order
			/*
			 * Collections.sort(topRatedMediaLinks, new Comparator<BusinessServicesDTO>() {
			 * 
			 * @Override public int compare(BusinessServicesDTO o1, BusinessServicesDTO o2)
			 * { return (int) (o2.getServiceRating() - o1.getServiceRating()); } });
			 * 
			 * Collections.sort(addedRecentMediaLinks, new Comparator<BusinessServicesDTO>()
			 * {
			 * 
			 * @Override public int compare(BusinessServicesDTO o1, BusinessServicesDTO o2)
			 * { return (o2.getCreatedDate().compareTo(o1.getCreatedDate())); } });
			 */
        	String type = listBservices.getType();
        	if(listBservices.getCategoryId() == null) {
    			if((type!= null && type.equalsIgnoreCase("Top Rated")) || type==null) {
    				MediaLinksDTO topRatedDto = new MediaLinksDTO();
    				topRatedDto.setType("Top Rated");
    				topRatedDto.setServiceList(topRatedMediaLinks);
    				data.add(topRatedDto);
    			}
    			
    			if((type!= null && type.equalsIgnoreCase("Near You")) || type==null) {
    				MediaLinksDTO nearYouDto = new MediaLinksDTO();
    				nearYouDto.setType("Near You");
    				nearYouDto.setServiceList(bServicesDTOList);
    				data.add(nearYouDto);
    			}			
    			
    			if((type!= null && type.equalsIgnoreCase("Favourites")) || type==null) {
    				MediaLinksDTO favouritesDto = new MediaLinksDTO();
    				favouritesDto.setType("Favourites");
    				favouritesDto.setServiceList(favouriteMediaLinks);
    				data.add(favouritesDto);
    			}
    			
    			if((type!= null && type.equalsIgnoreCase("Added Recently")) || type==null) {
    				MediaLinksDTO addedRecently = new MediaLinksDTO();
    				addedRecently.setType("Added Recently");
    				addedRecently.setServiceList(addedRecentMediaLinks);
    				data.add(addedRecently);
    			}
        	}
			
			if((type!= null && type.equalsIgnoreCase("Category Media Links")) || listBservices.getCategoryId() != null || type==null) {
				MediaLinksDTO categoryLinksDto = new MediaLinksDTO();
				categoryLinksDto.setType("Category Media Links");
				List<MediaLinksDTO> bServices = new ArrayList<>();
				for(Entry<String, List<BusinessServicesDTO>> entry : categoryMediaLinks.entrySet()) {
					MediaLinksDTO categoryDto = new MediaLinksDTO();
					categoryDto.setType(entry.getKey());
					SubCategory subCat = subCategoryDao.findBySubcategoryId(entry.getValue().get(0).getCategoryId());
					categoryDto.setCategoryId(subCat.getCategory().getCategoryId());
					categoryDto.setServiceList(entry.getValue());
					bServices.add(categoryDto);
				}
				categoryLinksDto.setList(bServices);
				data.add(categoryLinksDto);		
			}
			...
			logger.error("error in listBusinessServices: "+e.getMessage());
        	dto.setResponseStatus("ERROR : "+ e.getMessage());
			e.printStackTrace();
		}
        return dto;
    }
